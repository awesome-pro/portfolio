...
export default function DeleteRepoButton({ id, name }: { id: string; name: string }) {
  ...
  async function handleDelete() {
    ...
    await supabase.from("contribution_targets").delete().eq("id", id);
    router.refresh();
  }
    ...
    return (
      <div className="flex items-center gap-2">
        <span className="text-xs text-ink-muted font-mono">
          Delete &quot;{name.slice(0, 25)}{name.length > 25 ? "…" : ""}&quot;?
        </span>
        <button
